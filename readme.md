...
1. Convert the output manifest file to JSON (Power BI currently can't import YAML). For example you can use https://github.com/bronze1man/yaml2json. To speed up the process, you could optionally remove all YAML elements from the output manifest except for `tree.outputs`, as that's the only element Power BI uses. Store the JSON in the created directory from previous step. On Windows we can use `Get-Content .\if-manifest-output.yaml | {yaml2json binary} > .\if-manifest-output.json` from Powershell. 
